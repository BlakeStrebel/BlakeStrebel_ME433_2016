...
i2c_read_multiple(char address, char reg, unsigned char * data, char length)
{
    ...
    for (i = 0; i < length; i++)    
    {
        data[i] = i2c_master_recv();    // store register values
        
        if (i == length - 1) {
           i2c_master_ack(1);   // finished reading
        }
        else {
