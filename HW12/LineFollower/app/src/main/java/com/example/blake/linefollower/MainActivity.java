package com.example.blake.linefollower;
...
public class MainActivity extends Activity implements TextureView.SurfaceTextureListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        thresholdControl = (SeekBar) findViewById(R.id.seek1);
        thresholdControl.setMax(200);
        threshold = 150;
        thresholdTextView = (TextView) findViewById(R.id.textView01);
        thresholdTextView.setText("The threshold is: "+threshold);

        setThresholdControlListener();
    }
    private void setThresholdControlListener() {
        thresholdControl.setOnSeekBarChangeListener(new OnSeekBarChangeListener() {
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
        ...
        parameters.setAutoWhiteBalanceLock(true);   // no autowhite balance
    ...
    public void onSurfaceTextureUpdated(SurfaceTexture surface) {
        ...
        if (c != null) {
            ...
            int startY[] = {50, 200, 350};
            int ii;
            for(ii = 0; ii<3; ii++)
            {
                int[] pixels = new int[bmp.getWidth()];

                bmp.getPixels(pixels, 0, bmp.getWidth(), 0, startY[ii], bmp.getWidth(), 1); // (array name, offset inside array, stride (size of row), start x, start y, num pixels to read per row, num rows to read)
                ...
                if (wbTotal <= 0) {
                    COM[ii] = bmp.getWidth() / 2;   //sets default COM
                } else {
                    COM[ii] = wbCOM / wbTotal;
                }

                // draw a circle where you think the COM is
                canvas.drawCircle(COM[ii], startY[ii], 5, paint1);
            }
